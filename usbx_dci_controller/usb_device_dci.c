...
#if ((defined(USB_DEVICE_CONFIG_MUSB)) && (USB_DEVICE_CONFIG_MUSB > 0U))
#include "usb_device_musb.h"
#endif
...
static const usb_device_controller_interface_struct_t s_UsbDeviceMUSBInterface = {
    USB_DeviceMUSBInit, USB_DeviceMUSBDeinit, USB_DeviceMUSBSend,
    USB_DeviceMUSBRecv, USB_DeviceMUSBCancel, USB_DeviceMUSBControl};
#endif
    ...
    uint8_t controllerId, const usb_device_controller_interface_struct_t **controllerInterface)
{
    ...
    if (kUSB_ControllerMUSB0 == controlerIndex)
    {
        *controllerInterface = (const usb_device_controller_interface_struct_t *)&s_UsbDeviceMUSBInterface;
...
usb_status_t USB_DeviceSetStatus(usb_device_handle handle, usb_device_status_t type, void *param)
{
    ...
    switch (type)
    {
#if ((defined(USB_DEVICE_CONFIG_MUSBHS) && (USB_DEVICE_CONFIG_MUSBHS > 0U)))
        ...
        case kUSB_DeviceStatusAddress:
            ...
            {
                //usb_echo("\rkUSB_DeviceControlPreSetDeviceAddress\n\r");
                if (NULL != param)
                {
                    status                                         = kStatus_USB_Success;
                    ((usb_device_struct_t *)handle)->deviceAddress = (uint8_t)(*(uint8_t *)param);
                    ((usb_device_struct_t *)handle)->state         = (uint8_t)kUSB_DeviceStateAddress;
            ...
            {
                //usb_echo("\rkUSB_DeviceControlSetDeviceAddress\n\r");

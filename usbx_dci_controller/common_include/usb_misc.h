...
#else
/* Change the USB_DATA_ALIGN_SIZE to 4, For the musb driver, the musb driver will do the memcpy for the
   ...
   the musb platform, please change the macro to 64 and put the transfer buffer into the USB dedicated RAM. */
#define USB_DATA_ALIGN_SIZE 4U
#endif

/* Due to the change of USB_DATA_ALIGN_SIZE from 64 to 4 on the musb platform, the size of variables defined by
   using this marco may be smaller on the musb platform. If users don't want the musb driver to do memcpy,

...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _ux_dcd_musb_endpoint_create                      PORTABLE C      */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
...
UX_DCD_MUSB_ED     *ed;
ULONG               musb_endpoint_index;
usb_device_endpoint_init_struct_t epInitStruct;
usb_device_endpoint_callback_struct_t epCallback;
usb_status_t status;


    /* The endpoint index in the array of the MUSB must match the endpoint number.  */
    musb_endpoint_index =  endpoint -> ux_slave_endpoint_descriptor.bEndpointAddress & ~UX_ENDPOINT_DIRECTION;

    musb_endpoint_index = musb_endpoint_index << 1;

    if ((endpoint -> ux_slave_endpoint_descriptor.bEndpointAddress & UX_ENDPOINT_DIRECTION) == UX_ENDPOINT_IN)
        musb_endpoint_index++;

    /* Fetch the address of the physical endpoint.  */
    ed =  &dcd_musb -> ux_dcd_musb_ed[musb_endpoint_index];

    /* Check the index range and endpoint status, if it is free, reserve it. If not reject this endpoint.  */
    if ((musb_endpoint_index < UX_DCD_MUSB_MAX_ED) && ((ed -> ux_dcd_musb_ed_status & UX_DCD_MUSB_ED_STATUS_USED) == 0))
    {

        /* We can use this endpoint.  */
        ed -> ux_dcd_musb_ed_status |=  UX_DCD_MUSB_ED_STATUS_USED;

        /* Keep the physical endpoint address in the endpoint container.  */
        endpoint -> ux_slave_endpoint_ed =  (VOID *) ed;

        /* Save the endpoint pointer.  */
        ed -> ux_dcd_musb_ed_endpoint =  endpoint;

        /* And its index.  */
        ed -> ux_dcd_musb_ed_index =  musb_endpoint_index;

        /* And its direction.  */
        ed -> ux_dcd_musb_ed_direction =  endpoint -> ux_slave_endpoint_descriptor.bEndpointAddress & UX_ENDPOINT_DIRECTION;

        /* Check if it is non-control endpoint.  */
        if (musb_endpoint_index != 0)
        {

            /* Initialize the endpoint structures.  */
            epCallback.callbackFn    = _ux_dcd_musb_transfer_callback;
            ...
            status = USB_DeviceInitEndpoint(dcd_musb -> handle, &epInitStruct, &epCallback);

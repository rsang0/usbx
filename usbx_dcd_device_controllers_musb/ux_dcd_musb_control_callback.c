...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _ux_dcd_musb_control_callback                     PORTABLE C      */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
                                              ...
                                              void *callbackParam)
{
    ...
    if (message->isSetup)
    {
        ...
        if (*transfer_request -> ux_slave_transfer_request_setup & UX_REQUEST_IN)
        {
            ...
            /* The endpoint is IN.  This is important to memorize the direction for the control endpoint
               in case of a STALL. */
            ed -> ux_dcd_musb_ed_direction  = UX_ENDPOINT_IN;
            /* Set the state to TX.  */
            ed -> ux_dcd_musb_ed_state =  UX_DCD_MUSB_ED_STATE_DATA_TX;
        ...
        else
        {
            ...
            /* The endpoint is OUT.  This is important to memorize the direction for the control endpoint
               in case of a STALL. */
            ed -> ux_dcd_musb_ed_direction  = UX_ENDPOINT_OUT;
                ...
                *(transfer_request -> ux_slave_transfer_request_setup + 7) == 0)
            {
                ...
                if (_ux_device_stack_control_request_process(transfer_request) == UX_SUCCESS)
                {
                    ...
                    ed -> ux_dcd_musb_ed_state =  UX_DCD_MUSB_ED_STATE_STATUS_RX;
            ...
            else
            {
                ...
                if (transfer_request -> ux_slave_transfer_request_requested_length > UX_SLAVE_REQUEST_CONTROL_MAX_LENGTH)
                {
                    ...
                    _ux_dcd_musb_endpoint_stall(dcd_musb, endpoint);
                ...
                else
                {
                    ...
                    /* Set the state to RX.  */
                    ed -> ux_dcd_musb_ed_state =  UX_DCD_MUSB_ED_STATE_DATA_RX;
                }
            }
        }
    }
    else
    {
        ...
        if (ed -> ux_dcd_musb_ed_state == UX_DCD_MUSB_ED_STATE_DATA_TX)
        {
            /* Arm Status transfer.  */
        ...
        if (ed -> ux_dcd_musb_ed_state == UX_DCD_MUSB_ED_STATE_DATA_RX)
        {
                ...
                transfer_request -> ux_slave_transfer_request_requested_length)
            {
                    ...
                    (transfer_length != endpoint -> ux_slave_endpoint_descriptor.wMaxPacketSize))
                {
                    ...
                    if (_ux_device_stack_control_request_process(transfer_request) == UX_SUCCESS)
                    {
                        ...
                        ed -> ux_dcd_musb_ed_state =  UX_DCD_MUSB_ED_STATE_STATUS_TX;

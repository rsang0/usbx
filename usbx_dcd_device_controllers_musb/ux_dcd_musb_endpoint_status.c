...
/*  FUNCTION                                               RELEASE        */
/*                                                                        */
/*    _ux_dcd_musb_endpoint_status                      PORTABLE C      */
/*                                                           6.1          */
/*  AUTHOR                                                                */
/*                                                                        */
...
UX_DCD_MUSB_ED      *ed;
    ...
    ed =  &dcd_musb -> ux_dcd_musb_ed[endpoint_index];

    /* Check the endpoint status, if it is free, we have a illegal endpoint.  */
    if ((ed -> ux_dcd_musb_ed_status & UX_DCD_MUSB_ED_STATUS_USED) == 0)
        return(UX_ERROR);

    /* Check if the endpoint is stalled.  */
    if ((ed -> ux_dcd_musb_ed_status & UX_DCD_MUSB_ED_STATUS_STALLED) == 0)
        return(UX_FALSE);
    else
        return(UX_TRUE);
}
